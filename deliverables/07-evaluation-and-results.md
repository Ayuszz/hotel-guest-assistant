...
## Production re-run with Cerebras as primary (2026-09-22, later the same day)

After switching the provider order to `cerebras (gpt-oss-120b) → gemini`, the same 18 requests were run against https://hotel-guest-assistant-lilac.vercel.app from a client in India, one per second, timing the full round trip through Vercel.

| Metric | Gemini-only run | Cerebras-primary run |
| --- | --- | --- |
| Correct outcomes | 17 / 17 | 17 / 17 |
| Median latency | 13.7 s | 2.9 s |
| p90 latency | 20.0 s | 7.4 s |
| Fastest model-backed reply | ~9 s | 1.4 s |
