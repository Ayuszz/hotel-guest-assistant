...
import { InMemoryConversationRepo, type ConversationRepo } from "@/server/conversationRepo";
...
/** A single-thread chat session: threads the server-assigned conversationId across calls, like the real client does. */
function session(userId = USER) {
  let conversationId: string | undefined;
  const ask = async (message: string, extra: Record<string, unknown> = {}, provider = new MockProvider()) => {
    const r = await handleChat({ conversationId, message, ...extra }, userId, { provider, repo, today });
    if ("conversationId" in r.body && r.body.conversationId) conversationId = r.body.conversationId;
    return r;
  };
  return { ask, id: () => conversationId };
}

describe("handleChat: validation", () => {
  it("rejects an empty body with 400 and a typed error", async () => {
    const r = await handleChat({}, USER, { repo, today, provider: new MockProvider() });
  ...
  it("rejects malformed availability params", async () => {
    const r = await handleChat({ availability: { checkIn: "10/10/2026", checkOut: "2026-10-12", adults: 2 } }, USER, { repo, today, provider: new MockProvider() });
    expect(r.status).toBe(400);
    expect(r.body.message).toMatch(/YYYY-MM-DD/);
  });
  it("rejects a conversationId from another user with 404, without revealing whether it exists", async () => {
    const owner = session(USER);
    await owner.ask("What time is check-in?");
    const r = await handleChat({ conversationId: owner.id(), message: "hi" }, OTHER_USER, { repo, today, provider: new MockProvider() });
...
describe("handleChat: knowledge questions (normal)", () => {
  it("answers check-in time from the knowledge base with sources", async () => {
    const r = await session().ask("What time is check-in?");
  ...
  it("answers the swimming pool question", async () => {
    const r = await session().ask("Does the hotel have a swimming pool?");
    expect(r.body.type).toBe("text");
    expect(r.body.message).toMatch(/infinity pool/i);
  });
  it("answers which room suits three guests", async () => {
    const r = await session().ask("Which room is suitable for three guests?");
    expect(r.body.type).toBe("text");
...
describe("handleChat: unsupported and ambiguous", () => {
  it("returns a fallback for a question outside the knowledge base", async () => {
    const r = await session().ask("Who won the cricket world cup?");
  ...
  it("returns an out-of-scope fallback when the router says unsupported", async () => {
    const r = await session().ask("Tell me a joke about the weather");
    expect(r.body.type).toBe("fallback");
  ...
  it("falls back when the model says the context does not answer the question", async () => {
    ...
    const r = await session().ask("Is it free?", {}, p);
    expect(r.body.type).toBe("fallback");
  });
  it("rejects an answer containing a figure that is not in the knowledge base (hallucination guard)", async () => {
    ...
    const r = await session().ask("How much is the pool?", {}, p);
    expect(r.body.type).toBe("fallback");
...
describe("handleChat: availability (tool calling)", () => {
  it("asks for missing details when dates are absent", async () => {
    const r = await session().ask("Do you have rooms available?");
  ...
  it("calls the tool when the model extracts all slots", async () => {
    const r = await session().ask("Any rooms from 2026-10-10 to 2026-10-12 for 2 adults?");
  ...
  it("runs the tool directly for a structured form submission, no LLM involved", async () => {
    ...
    const r = await handleChat({ availability: { checkIn: "2026-10-10", checkOut: "2026-10-12", adults: 3 } }, USER, { provider: failing, repo, today });
    expect(r.status).toBe(200);
    expect(r.body.type).toBe("availability");
  });
  it("returns a 400 with a readable message for a past check-in from the form", async () => {
    const r = await handleChat({ availability: { checkIn: "2026-01-01", checkOut: "2026-01-03", adults: 2 } }, USER, { provider: new MockProvider(), repo, today });
    expect(r.status).toBe(400);
    expect(r.body.message).toMatch(/past/);
  });
  it("remembers slots across turns and only asks for what is still missing", async () => {
    const s = session();
    await s.ask("I want to book for 2 adults");
    const r = await s.ask("Any rooms from 2026-10-10 to 2026-10-12?");
    expect(r.body.type).toBe("availability");
...
describe("handleChat: conversation follow-ups", () => {
  it("passes DB-backed history to the model so 'how much is it' can be resolved", async () => {
    let seenHistory = 0;
    ...
    const s = session();
    await s.ask("Which room is suitable for three guests?", {}, p);
    const r = await s.ask("How much is it per night?", {}, p);
    expect(seenHistory).toBe(2);
    expect(r.body.type).toBe("text");
    expect(r.body.message).toMatch(/9200/);
  });
  it("keeps every turn under the same conversationId across the session", async () => {
    const s = session();
    const first = await s.ask("Is breakfast included?");
    const second = await s.ask("And the pool?");
    expect("conversationId" in first.body && first.body.conversationId).toBe(s.id());
    expect("conversationId" in second.body && second.body.conversationId).toBe(s.id());
  });
});
...
describe("handleChat: model failure and fallback", () => {
  it("returns an llm_unavailable fallback when the model times out on a knowledge question", async () => {
    const r = await session().ask("Is breakfast included?", {}, new MockProvider({ fail: "timeout" }));
    expect(r.status).toBe(200);
    expect(r.body.type).toBe("fallback");
  ...
  it("still routes an obvious availability request to the form when the model is down", async () => {
    const r = await session().ask("Do you have any rooms available next weekend?", {}, new MockProvider({ fail: "network" }));
    expect(r.body.type).toBe("clarification");
  });
  it("handles malformed model JSON as a fallback rather than a crash", async () => {
    const r = await session().ask("What is the cancellation policy?", {}, new MockProvider({ fail: "malformed" }));
    expect(r.body.type).toBe("fallback");
  });
});
